...
order by t.vn asc`,
    ...
    getAllSm: (bdate, callBack) => {
        pool.query(
            `select (@cnt := @cnt + 1) AS pp_special_id,t.vn,t.smoking_id as pp_special_type_id,(select (select doctorcode from opduser o where o.loginname = staff ) from opdscreen_cc_history where vn = t.vn and vstdate = ?  GROUP BY vn) as doctor,'1' as pp_special_service_place_type_id
